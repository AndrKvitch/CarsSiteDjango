...
def about(request):
    contact_list = Cars.objects.all()
    paginator = Paginator(contact_list, 3)

    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    return render(request, 'cars/about.html', {'page_obj': page_obj, 'menu': menu, 'title': 'О сайте'})
...
#     else:
#         form = AddPostForm()
#     return render(request, 'cars/addpage.html', {'form': form, 'menu': menu, 'title': "Добавить статью"})
...
class CarsCategory(DataMixin, ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        context = super().get_context_data(**kwargs)
        c = Category.object.get(slug=self.kwargs['cat_slug'])
